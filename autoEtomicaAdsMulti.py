...
def waitEquil(B = False):
    ...
    while notequil:
        ...
        meanAd = getmeanAds(BPresent = B)        
        meanAds.append(meanAd[0])
        meanAdsB.append(meanAd[1])
...
pstart = 1e-6
ptotal = 2e-3
Pdat = np.linspace(pstart, ptotal, 10)
PB = Pdat[::-1]
Pdat, PB = np.around(np.log10(Pdat), decimals=2), np.around(np.log10(PB), decimals=2)
...
for i, P in enumerate(Pdat):          
    ...
    if BPresent == True:
        setP(PB[i], B=BPresent)
        setEps(epsB, B=BPresent)    
    
    print(f'P_A: {P}. P_B: {PB[i]}')
    ...
    with pd.ExcelWriter(path, engine='openpyxl', mode='a') as writer:
        ...
        dfT = df.T
        name = 'T' + str(T) + '-P' + str(abs(P)) + '-e' + str(eps) + '-PB' + str(abs(PB[i]))
